...
float scene_SDF(out Intersection isect) {
  ...
  float ridge = box_round_SDF(right_hill_2_deform, vec3(55.0f, -70.0f, 500.0f), vec3(500.0f, 60.0f, 100.5f), 5.95f);
    
  float trees[10];
  for (int i = 0; i < 20; ++i) {
      ridge = min(ridge,smoothUnion_SDF(
        cylinder_rounded_SDF( isect.p, vec3(-500.0f + 60.0f * float(i), 1.0f + sin(isect.p.x / 25.0f) * 10.5f, sin(isect.p.x / 25.0f) * 15.0f + 425.0f), 
        1.0f,  1.0f, 20.0f ), cylinder_rounded_SDF(isect.p, vec3(-500.0f + 60.0f * float(i) + sin(u_Time / 13.0f) * 2.0f, 20.0f + sin(isect.p.x / 25.0f) * 10.5f, sin(isect.p.x / 25.0f) * 15.0f + 425.0f), 
        7.5f * fbm_val,  7.5f * fbm_val, 2.0f ), 4.5f));
  }
  return ridge;
}
...
vec3 shadeIntersection(Intersection isect) {
  ...
  //material_color = vec3(0.6, 0.85, 0.3);
    ...
    material_color = vec3(0.35,0.45,1.4);
  float t = abs(isect.p.y / 125.0f);
  t = bias(t, 0.85f);
  t = gain(t, 0.85f);
  material_color = mix(vec3(0.4, 0.85, 0.3), material_color, t);
