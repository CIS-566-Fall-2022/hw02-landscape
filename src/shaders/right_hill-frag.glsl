...
const DirectionalLight gi_light = DirectionalLight(normalize(vec3(-5.0, 0, -5.0)), vec3(.21, .02, .005));
...
vec3 shadeIntersection(Intersection isect) {
  ...
  if (isect.material_ID == MAT_RIGHT_HILL) {
  ...
  t = -(isect.p.y - 35.0f) / 35.0f;
  if (isect.p.y > 0.0f) {
    material_color = mix(vec3(0.5,0.74, 1.55), material_color, t);
  }
