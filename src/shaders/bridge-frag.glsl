...
float scene_SDF(out Intersection isect) {
  float worley_val = bias(1.0f - worley3D(isect.p, 0.35f), 0.9598);
  float eldin_bridge = eldin_bridge_SDF(isect.p + vec3(worley_val * 1.5f * sign(isect.p.x), -worley_val * 5.0f, worley_val * 4.5f), vec3(0.0f, -10.0f, 35.0f));
  isect.distance_to_surface = worley_val;

  float min_distance = eldin_bridge;
...
vec3 shadeIntersection(Intersection isect) {
  ...
  float t = (isect.distance_to_surface + (-isect.p.y) / 50.0f) / 2.0f;
  material_color = mix(vec3(0.2,0.2,0.2), vec3(1,1,1), 1.0f - t);
