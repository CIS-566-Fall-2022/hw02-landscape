...
float scene_SDF(out Intersection isect) {
  ...
  return plane_SDF(isect.p + vec3(0, sin(u_Time / 15.0f + isect.p.x / 20.0f), 0), vec3(0,1,0), 100.0f);
}
...
vec3 shadeIntersection(Intersection isect) {
  ...
  if (isect.material_ID == MAT_RIVER) {
    float worley_val = worley3D(isect.p + vec3(0, 0,u_Time / 15.0f) + vec3(sin((u_Time / 2.0f) + isect.p.x) * 0.25f, 0,0), 0.25f);
    vec3 river_color = vec3(0, (1.0f - worley_val) * 0.4f, (1.0f - worley_val) * 1.2f);
    material_color = mix(river_color, vec3(0.78, 0.98, 1.98), worley_val * 0.8f);
  }
